...
// --- boot ---
Promise.all([
  fetch('./data-part1.json').then(r => { if (!r.ok) throw new Error('data-part1.json ' + r.status); return r.json(); }),
  fetch('./data-part2.json').then(r => { if (!r.ok) throw new Error('data-part2.json ' + r.status); return r.json(); }),
  fetch('./data-part3.json').then(r => { if (!r.ok) throw new Error('data-part3.json ' + r.status); return r.json(); })
])
  .then(([p1, p2, p3]) => {
    DATA = Object.assign({}, p3, { templeIndex: [...p1.templeIndex, ...p2.templeIndex] });
    renderAbodes();
    renderVenues('');
    renderWorld();
    renderLives();
