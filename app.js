...
// --- boot ---
Promise.all([1,2,3,4,5,6,7].map(n =>
  fetch(`./data-part${n}.json`).then(r => { if (!r.ok) throw new Error('data-part' + n + '.json ' + r.status); return r.json(); })
))
  .then(([p1, p2, p3, p4, p5, p6, p7]) => {
    DATA = {
      templeIndex: [...p1.templeIndex, ...p2.templeIndex, ...p3.templeIndex],
      abodes: p3.abodes,
      anubhuti: p4.anubhuti,
      venues: p5.venues,
      worldTemples: p5.worldTemples,
      stories: p6.stories,
      sangam: p6.sangam,
      texts: p7.texts
    };
